...
void Split(const char* sourceFileName, size_t partsCount)
{
	// добавление к пути файла окончания ".part"
	char destFileName[200];
	strcpy(destFileName, sourceFileName);
	strcat(destFileName, ".part");

	// открыть файл для чтения
	FILE* sourceFile = fopen(sourceFileName, "rb");
	if (sourceFile != nullptr)
	{
		// переместиться на конец файла
		fseek(sourceFile, 0, SEEK_END);

		// получить номер байта, где стоит указатель для чтения (размер файла)
		...
		// выделение памяти для хранения файла
		char* buffer = new char[size];

		if (buffer != nullptr)
		{
			// считать всё из исходного файла
			fread(buffer, 1, size, sourceFile);

			// вычислить приблизительный размер части
			size_t partSize = size / partsCount;

			// если количество частей оказалось больше, чем размер файла
			if (partSize == 0)
				return;

			// если деление произошло с остатком
			if (partSize * partsCount != size)	// size % partsCount != 0
			{
				// размер обычной (не последней) части увеличить на 1
				partSize++;
			}

			// цикл по частям, которые нужно сохранить
			for (size_t i = 0; i < partsCount; i++)
			{
				// строковый номер части
				char partNum[20];

				// конвертировать номер части из int в строку
				_itoa(i + 1, partNum, 10);

				// конечное имя файла с частью
				char currentFileName[200];
				strcpy(currentFileName, destFileName);
				strcat(currentFileName, partNum);

				// открыть файл для записи
				FILE* destFile = fopen(currentFileName, "wb");
				if (destFile != nullptr)
				{
					// записывается последняя часть
					if (i == partsCount - 1)
					{
						// размер последней части
						size_t lastPartSize = size - partSize * (partsCount - 1);
						fwrite(buffer + i * partSize, 1, lastPartSize, destFile);
					}
					else	// записывается не последняя часть				
					{
						// записать в файл кусок исходного файла, начиная от места начала текущей части
						fwrite(buffer + i * partSize, 1, partSize, destFile);
					}
				}
				fclose(destFile);
			}
		...
		else cout << "File is too big!!!" << endl;
...
void Join(const char* folderPath, const char* sourceFileName, size_t partsCount, const char* destFileName)
{
	// полный путь к исходным файлам
	char sourceFilePath[200];

	// скопировать путь к исходной папке
	strcpy(sourceFilePath, folderPath);
	strcat(sourceFilePath, "\\");

	// добавить имя исходных файлов
	strcat(sourceFilePath, sourceFileName);
	strcat(sourceFilePath, ".part");

	// открыть результирующий файл для записи
	FILE* destFile = fopen(destFileName, "wb");
	if (destFile != nullptr)
	{
		// цикл по частям, которые нужно соединить
		for (size_t i = 0; i < partsCount; i++)
		{
			// строковый номер части
			char partNum[20];
			...
			// конечное имя файла с частью
			char currentFileName[200];
			strcpy(currentFileName, sourceFilePath);
			strcat(currentFileName, partNum);

			cout << currentFileName << "  ";

			// открыть файл для чтения
			FILE* sourceFile = fopen(currentFileName, "rb");
			if (sourceFile != nullptr)
			{
				// переместиться на конец файла
				fseek(sourceFile, 0, SEEK_END);
				...
				// выделение памяти для хранения одной части
				char* buffer = new char[size];
				...
				if (buffer != nullptr)
				{
					...
					// сохранить считанную часть в результирующий файл
...
void main()
{
	...
	*/
	...
	if (file1 != nullptr && file2 != nullptr)
	{
		...
		// пользователь вводит имя файла и количество частей, программа делит файл на части
		...
		cin >> path;
		cout << "Enter file name to join: ";
		cin >> filename2;
		cout << "Enter destination full name: ";
		cin >> destFilename;
		cout << "Enter amount of parts: ";
		cin >> partsCount;

		Join(path, filename2, partsCount, destFilename);*/
